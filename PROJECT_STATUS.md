  ...
  components/
    ...
    3d/
      VisualKit.jsx     — reusable low-poly primitives: DungeonWall, ArchedDoorway,
                           TreasureChest (see §1 decisions 49, 50, 52) - DungeonWall is in 15
                           rooms now, ArchedDoorway/TreasureChest still just 1 each
```
...
20. Finish adopting `VisualKit.jsx` (§1 decisions 49, 50, 52) across the rest of the game — `DungeonWall` is now wired into 15 rooms (Troll Room, Cellar, Egyptian Room, Chasm Room, Cold Passage, Cyclops Room, East of Chasm, EW Passage, Gallery (side walls only), Loud Room, Mirror Room, Narrow Passage, NS Passage, Twisting Passage, Winding Passage); `ArchedDoorway`/`TreasureChest` are each still only in South Temple/Egyptian Room respectively. Deliberately skipped so far: Gallery's back wall (painting clipping risk) and Studio (paint-splatter clipping risk, same reasoning, whole room deferred) - both need either a repositioned prop or a `DungeonWall` option to suppress protrusion before they're safe to convert. The remaining ~55 rooms (mostly surface/house interiors that shouldn't get a *stone* wall at all, plus whatever's left underground) are still open
