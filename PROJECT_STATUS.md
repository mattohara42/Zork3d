...
| 2 | "Theater stage" transition model: camera never animates between rooms; each room's geometry is torn down and rebuilt fresh around a fixed camera | Avoids hand-rolled `requestAnimationFrame` easing loops; matches the discrete, room-to-room nature of text-adventure movement. **Amended by §1 decision 55**: the camera's *position* can now slide per room via an opt-in `cameraOffset`, but this is still the same fixed-rig model at heart — rotation is still set once and frozen, geometry is still rebuilt fresh, and the ~70 rooms that don't set `cameraOffset` are completely unaffected |
...
| 48 | Graduated `hint` command, scoped to the Maze only | User asked for a maze hint system specifically. Modeled on Infocom's own printed InvisiClues books (vague nudge first, sharper on repeat asks) rather than a flat walkthrough, so it doesn't just hand over the solution on the first try. `mazeHintLevel` persists through save/restore, resets on `restart`; the command is a free action (no `incrementMoves`) and does nothing outside a maze-blob room. The tier-3 text (real, specific directions) was wrong on the first draft - `west, up, down` from the skeleton's room lands at Maze-11, not the Grating Room itself, missing a final `east` hop - caught only by actually walking every direction in a live browser rather than trusting the BFS math (the same math from decision #47) that generated it. Both routes (to the Grating Room, and separately to the Cyclops Room) are now verified end-to-end, not just plausible-looking |
| 49 | `src/components/3d/VisualKit.jsx` holds reusable low-poly primitives (`DungeonWall`, `ArchedDoorway`, `TreasureChest`) as a shared kit, not built inline per room | User asked for exactly this shape: a single file of procedural, parametric components rather than one-off geometry copy-pasted into each room file. All three share a `KitMaterial` helper (`roughness=0.9`, `flatShading=true` on every mesh, per spec) so the stylized look stays consistent without repeating those two props everywhere. `DungeonWall` clusters 4 hand-picked (not randomized) darker blocks, sized slightly wider than the wall itself along its thickness axis so they protrude from whichever face ends up facing the camera - the component never needs to know which side that is. `ArchedDoorway` fakes a curved arch out of straight box "voussoirs" fanned around a pivot (rotating the wrapping group, not the mesh, is what angles each block tangent to the curve) - a classic low-poly technique, not curved geometry. `TreasureChest`'s lid hinges open by rotating a pivot group at its rear-top edge, the same "swap state, not a whole new mesh" pattern the mailbox's lid already uses elsewhere in this game. Verified visually via a throwaway `DevPreview.jsx`/`main.jsx` swap (screenshotted in context, then fully reverted before committing - matches the standing practice noted just above this table). Wired into four flagship rooms as an initial pass (§1 decision 50) - the remaining ~66 rooms are still plain primitives, real follow-up work (§3 backlog #20) |
| 50 | Adopted `VisualKit` in four flagship rooms rather than a full sweep | `DungeonWall` is a direct drop-in for `TrollRoom`/`Cellar`'s existing plain-box walls (same position/size args). `EgyptRoom`'s coffin now renders as a `TreasureChest` with both colors pushed gold (`woodColor`/`goldColor`) so it still reads as "solid-gold," not a wood chest with gold accents - the room's existing click/open dispatch stays on a wrapping `<group>`, since `TreasureChest` itself has no interaction logic of its own. `SouthTemple` gets a new background `ArchedDoorway` (not a replacement - the room had no wall geometry before) with a dim stone `openingColor` instead of the kit's near-black default, since this room is naturally lit and a stark black hole would've read as a void. Picking a small, verifiable set over rewriting all ~70 rooms at once is what surfaced decision #51 below - a real, unrelated bug that a blind mass find-and-replace wouldn't have caught nearly as fast |
| 51 | `ViewportCanvas`'s background color now also checks `environment === 'underground'`, not just the lamp-derived `isDark`/`isUnderground` booleans | Those two booleans are both derived from `room.dark` alone, so a naturally-lit underground room (`dark: false`, ONBIT in the source - the Gallery, both Temples, the Dam rooms) fell through to the same `SKY_COLOR` branch a bright outdoor room uses, showing a sky gradient bleeding through any open exit. See §5 for how this was caught - a real, previously-shipped bug unrelated to the VisualKit work in progress at the time |
| 52 | Second `DungeonWall` adoption batch: 11 more rooms (22 wall meshes), all direct side-wall drop-ins except two rotated back walls | Targeted every remaining room using the same thin-box wall pattern (`grep`'d for it) that's actually underground/stone - explicitly excluded Attic/Kitchen/Living Room/Studio (surface or plaster/wood interiors, not dungeon stone by the source's own description) and skipped Gallery's *back* wall specifically (its stone-block protrusions would clip through the painting hanging flush against it at `z=-4.4`; same reasoning applied to Studio's paint splatters, deferring that whole room). `LoudRoom` and Gallery's untouched-back-wall case surfaced a real technique worth naming: `DungeonWall`'s default shape is thin-along-local-X/long-along-local-Z (a *side* wall); a *back* wall (thin-in-Z, spanning the room's width in X) needs `rotation={[0, Math.PI/2, 0]}` rather than swapped width/depth args, since the stone-block sizing math is written in terms of the component's own local axes. Verified all 12 affected rooms live (no console errors, no clipping) before committing |
| 53 | Added a `protrude` prop to `DungeonWall` (defaults `true`, matching all existing usage) rather than special-casing Gallery/Studio permanently | Solves decision #52's deferral properly instead of leaving those two walls plain forever: `protrude={false}` sizes the stone blocks *narrower* than the wall (`width * 0.7`) instead of wider, so they sit fully recessed within its thickness - visible via the color contrast alone, no 3D bump to clip through a flush-mounted prop. Used for Gallery's back wall (the painting) and all three of Studio's walls (the paint splatters) - both rooms are now converted, closing out that part of backlog #20. A generic "flatten the protrusion, don't remove the detail" toggle felt like the right fix rather than a one-off workaround in either room file, since the same clash (something mounted flush against a stone wall) is exactly the kind of thing other rooms could hit too |
| 54 | Adopted `ArchedDoorway` in Entrance to Hades, replacing its plain post-and-lintel gate | The source's own text calls this "a large gateway" - a stronger, more literal match than South Temple's background archway (§1 decision 50), which had no specific text to match at all. Reused the kit's default near-black opening as-is (unlike South Temple's dim-stone override) since this room is genuinely dark, no ONBIT. Also gave Treasure Room's back wall `DungeonWall` (rotated, full protrusion - the thief/chalice/canary sit far enough from that wall for the 3D bump to be safe) |
| 55 | `CameraController.jsx` eases the camera's *position* toward a per-room `cameraOffset` (default `[0,0,0]`) every frame via `useFrame` + three separate `THREE.MathUtils.lerp` calls (one per axis), instead of the camera snapping straight to it | User asked for exactly this: `useFrame`, standard vector lerping via `MathUtils.lerp`, sliding rather than snapping on room change. Deliberately position-only - the camera's rotation is still set once at mount by `ViewportCanvas`'s `FixedCameraRig` (`lookAt(0,0,-1)`) and never touched again, so "always facing -Z" stays true regardless of where along that axis the camera actually sits; a sliding position needs no corresponding sliding look-at to still read correctly. The per-frame lerp factor `t` is `Math.min(MAX_STEP, delta * LERP_SPEED)`, not just `delta * LERP_SPEED` clamped to 1 - caught live, not by inspection: a temporary `window.__debugCameraPos` hook sampled via Playwright showed the camera landing exactly on its target on the very first frame after a room change, because mounting a whole new room's geometry can stall the render loop long enough that the next frame's `delta` alone covers the entire intended distance, collapsing the "slide" into an instant snap. Capping the step size at `MAX_STEP=0.2` guarantees the slide always plays out over multiple frames no matter how choppy the triggering frame was; re-measured afterward and confirmed genuine gradual convergence. `cameraOffset` defaults to `[0,0,0]` (`ZERO_OFFSET` in `ViewportCanvas.jsx`) for every room that doesn't set one - the ~70 untouched rooms keep exactly the fixed-origin camera they were always built around. Four rooms tailored so far, all small forward/upward dollies rather than dramatic moves: `trollRoom` `[0,0,-0.4]`, `egyptRoom` `[0,0,-0.3]` (both stepping in on the room's antagonist/focal object), `treasureRoom` `[0,0.06,-0.35]` and `southTemple` `[0,0.05,-0.3]` (a slight rise plus forward step for the two rooms felt to deserve the most presence - the game's payoff room and its altar) |
...
- Tests: `vitest` (`npm test`) — 34 cases against `useGameState`/`mapLayout`, see §1 decisions 45, 47, 48
...
src/
  ...
  state/
    ...
    __tests__/             — Vitest regression suite (movement/combat/persistence/puzzles/hints)
  components/
    ...
    CameraController.jsx — eases camera position toward each room's optional
                           `cameraOffset` instead of snapping (see §1 decision 55)
    ...
    3d/
      VisualKit.jsx     — reusable low-poly primitives: DungeonWall, ArchedDoorway,
                           TreasureChest (see §1 decisions 49-54) - DungeonWall is in 17
                           rooms now, ArchedDoorway in 2, TreasureChest in 1 (see backlog #20
                           for why that one stays at 1)
```
...
- Meta: `look`, `inventory`/`i`/`inv`, `help`, `score`, `diagnose` (real health/death count, not the source's wound-severity numbers - §1 decision 44), `hint` (Maze only, graduated - §1 decision 48, not sourced from the ZIL text since it's a new meta feature), `light lamp`/`turn on lamp`, `turn off lamp`/`extinguish lamp`/`douse lamp`, `ulysses`/`odysseus` (deliberately not listed in `help` - it's a discoverable secret in canon too), `restart` (asks for Y/N confirmation mid-game - §1 decision 36), `save`/`restore` (one `localStorage` slot - §1 decision 35)
...
- **Hints** (`hint` command, `useGameState`'s `hintVerb`/`MAZE_HINTS`) -
  scoped to the Maze specifically (the one puzzle the user asked for a
  hint system on), graduated InvisiClues-book style: vaguer on the first
  ask ("leave something behind to tell rooms apart"), more specific on
  the second (there's a skeleton/key and a locked grate), and a real,
  verified walkthrough on the third (exact directions to the key, to the
  Grating Room, and the alternate Cyclops Room route). It's a free action
  (no `incrementMoves`) and does nothing outside the Maze. `mazeHintLevel`
  persists through save/restore, resets on `restart`. The tier-3 text was
  wrong on the first pass (missing the final `east` hop into the actual
  Grating Room - `west, up, down` alone only reaches Maze-11, next door)
  and only caught by actually walking every direction in a live browser
  rather than trusting the BFS math that generated it - both routes are
  now verified end-to-end (reaches the Grating Room and successfully
  unlocks/opens the grate; separately reaches the Cyclops Room).
...
20. Finish adopting `VisualKit.jsx` (§1 decisions 49, 50, 52-54) across the rest of the game — `DungeonWall` is now wired into 17 rooms (the 15 from before, plus Gallery's back wall and all of Studio, both using `protrude={false}`, plus Treasure Room). `ArchedDoorway` is in 2 rooms (South Temple, Entrance to Hades); `TreasureChest` is deliberately still just 1 (Egyptian Room's coffin) - there is no other "container" item anywhere in this game's actual content, so a second use would mean inventing one just to show off the component, which the accuracy-first policy (§1 decision 3) rules out. The remaining ~53 rooms are mostly surface/house interiors that shouldn't get a *stone* wall at all; whatever's left underground and unconverted is still open
...
- **Naturally-lit underground rooms (Gallery, both Temples, the Dam rooms - ONBIT in the source) showed a bright outdoor sky background bleeding through open exits, instead of underground black.** `isDark`/`isUnderground` in `useGameState` are both derived from `room.dark`, and these rooms have `dark: false` (they're lit, just not by the lantern), so both booleans came out false and `ViewportCanvas`'s `backgroundColor` fell through to `SKY_COLOR` - the same branch a bright outdoor surface room hits. Had shipped unnoticed since the visual atmosphere pass (§1 decision 46) added the `<Sky>` dome that made the leak obvious; caught only when adding South Temple's archway prompted an actual screenshot of that specific room. Fixed by also checking `environment === 'underground'` in the background calculation, not just the two lamp-derived booleans.
