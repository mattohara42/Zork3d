  ...
  components/
    ...
    3d/
      VisualKit.jsx     — reusable low-poly primitives: DungeonWall, ArchedDoorway,
                           TreasureChest (see §1 decisions 49-50) - used by 4 rooms so far
```
...
20. Finish adopting `VisualKit.jsx` (§1 decisions 49-50) across the rest of the game — `DungeonWall`/`ArchedDoorway`/`TreasureChest` are wired into 4 flagship rooms so far (Troll Room, Cellar, Egyptian Room, South Temple). The remaining ~66 rooms still use plain single-box walls; swapping them in room-by-room is real, ongoing work, not a one-line change
