  ...
  components/
    ...
    3d/
      VisualKit.jsx     — reusable low-poly primitives: DungeonWall, ArchedDoorway,
                           TreasureChest (see §1 decision 49) - not yet used by any room
```
...
20. Adopt `VisualKit.jsx` (§1 decision 49) in actual rooms — `DungeonWall`, `ArchedDoorway`, and `TreasureChest` exist and are visually verified in isolation, but no room component uses them yet. Swapping them in for the plain single-box walls/doorways currently used across ~70 rooms is real, room-by-room follow-up work, not a one-line change
